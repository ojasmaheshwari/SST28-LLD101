import java.util.*;

public class StudentValidator {
	static public List<String> validate(final StudentRecord studentRecord) {
		String name = studentRecord.name;
		String email = studentRecord.email;
		String phone = studentRecord.phone;
		String program = studentRecord.program;

		List<String> errors = new ArrayList<>();
		if (name.isBlank())
			errors.add("name is required");
		if (email.isBlank() || !email.contains("@"))
			errors.add("email is invalid");
		if (phone.isBlank() || !phone.chars().allMatch(Character::isDigit))
			errors.add("phone is invalid");
		if (!(program.equals("CSE") || program.equals("AI") || program.equals("SWE")))
			errors.add("program is invalid");

		return errors;
	}

	static public void printValidationErrors(final List<String> errors) {
		if (!errors.isEmpty()) {
			System.out.println("ERROR: cannot register");
			for (String e : errors)
				System.out.println("- " + e);
		}
	}
}
